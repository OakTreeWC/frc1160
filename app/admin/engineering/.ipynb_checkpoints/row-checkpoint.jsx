...
export default function Row(params) {
    ...
    function submitEdit() {
        ...
        cab.grade=grade;
        cab.years=years;
        editCabinet(cab.sort,name,pos,grade,years);
        setSelected(false);
    }
          ...
          >
            {
                !selected && (
                    <>
                    <div className="col-span-3">{cab.name}</div>
                    <div className="col-span-2">{cab.position}</div>
                    <div className="col-span-2">{cab.grade}</div>
                    <div className="col-span-2">{cab.years}</div>
                    </>
                )
            }
            {
                selected && (
                    <>
                        <div contentEditable suppressContentEditableWarning ref={nameInput} className="col-span-3 text-center border-2 border-gray-500 rounded mx-3">{cab.name}</div>
                        <div contentEditable suppressContentEditableWarning ref={posInput} className="col-span-2 text-center border-2 border-gray-500 rounded mx-3">{cab.position}</div>
                        <div contentEditable suppressContentEditableWarning ref={gradeInput} className="col-span-2 text-center border-2 border-gray-500 rounded mx-3">{cab.grade}</div>
                        <div contentEditable suppressContentEditableWarning ref={yearsInput} className="col-span-2 text-center border-2 border-gray-500 rounded mx-3">{cab.years}</div>
                    </>
                )
            }
            <div className="col-span-3 flex flex-col py-2 space-y-2 justify-center items-center">
              ...
              <form action={deleteCabinet} className="flex justify-center items-center">
                <input type="hidden" name="name" value={cab.name} />
                <input type="hidden" name="years" value={cab.years} />
                <input type="hidden" name="grade" value={cab.grade} />
                <input type="hidden" name="position" value={cab.position} />

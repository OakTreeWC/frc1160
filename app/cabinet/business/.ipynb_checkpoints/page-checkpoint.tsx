                        ...
                        return (
                            <div key={member.name} className={clsx(`flex flex-col space-y-7 md:space-y-0 md:flex-row flex-wrap justify-center py-7 px-10 w-full md:px-0`,{'bg-[#a9c4e6]/67 md:bg-inherit':bg===1})}>
                                <div className="flex flex-col md:basis-1/4 items-center justify-center space-y-5">
                                    <Image src={"/cabinet/business/"+member.name.toLowerCase().replace(/\s+/g, '')+".jpg"} width={410} height={615} loading="eager" alt={member.name}></Image>
                                </div>
                                <div className="flex flex-col md:basis-3/4 items-left pl-8 space-y-2">

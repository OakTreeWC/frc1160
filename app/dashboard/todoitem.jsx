...
export default function ToDoItem({id, deleteTodoItem, removeWorkerTodoItem, acceptTodoItem, dismissTodoItem, updateTodoStatus, getComments, createComment, date, time, task, tasknote, creator, serverstatus, worker, self}) {
    ...
    return (
        <div className={clsx("w-full border-2 my-4 p-5 rounded-2xl", {"bg-red-500/20 border-red-500/30": status===0, "bg-yellow-500/20 border-yellow-500/30": status===1, "bg-green-500/20 border-green-500/30": status===2})} >
            ...
            {comment && (
                <RemoveScroll>
                <div className="fixed inset-0 flex justify-center items-center bg-black/35 p-6 overflow-y-auto z-50">
                    <div className="bg-white md:w-[800px] w-full p-7 text-black rounded-2xl flex flex-col max-h-[65vh] gap-6 overflow-hidden">

                    <span className="flex flex-row justify-between items-center w-full">
                        <button
                        className="p-2.5 bg-blue-500/30 hover:bg-blue-500/90 transition duration-300 ease-out rounded-xl hover:fill-white hover:cursor-pointer"
                        title="Reload Messages"
                        onClick={() => reloadComments()}
                        >
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" ref={reloadSVG} className="w-5 h-5">
                        ...
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 384 512" className="h-10 w-10 p-2 hover:cursor-pointer hover:fill-white transition hover:bg-blue-500/40 duration-300 ease-out rounded-xl">
                        ...
                        <div className="flex flex-col justify-start items-start space-y-5 bg-gray-300 rounded-lg px-5 py-10 text-left w-full flex-1 min-h-0 overflow-y-auto">

                        {comments.map((comment, index) => (
                            <div key={index} className="w-full flex flex-row gap-5">
                            <Image
                                src={comment.commentorPfp}
                                width={96}
                                height={96}
                            ...
                            <div className="flex flex-col grow gap-3">
                                ...
                                <span className="font-medium text-xl/7">
                                    {comment.commentorName}
                                </span>
                                <span className="font-normal text-sm p-1 px-2 rounded-md text-gray-600 bg-gray-400/50">
                                    {comment.commentorRole.charAt(0).toUpperCase() +
                                    comment.commentorRole.slice(1)}
                                </span>
                                </div>
                        ...
                        action={(formData) => {
                            ...
                            ...comments,
                            {
                                commentorName: self.name,
                                commentorPfp: self.image,
                                commentorRole: self.role,
                        ...
                        <input
                            placeholder="Comment"
                            name="comment"
                            className="border-2 border-gray-400 p-2 rounded-lg grow"
                        />
            ...
            {note && (
                <RemoveScroll>
                    <div className="fixed flex justify-center bg-black/35 items-center top-0 left-0 w-full h-screen z-50">
